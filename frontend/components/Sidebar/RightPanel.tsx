...
//         totalDelivery: result.summary?.delivery?.[0] || route.delivery?.[0]
//             .filter(step => step.type === 'job')
//             .reduce((sum, step) => {
//                 const currentLoad = step.load?.[0] || 0;
//                 const nextLoad = route.steps[route.steps.indexOf(step) + 1]?.load?.[0] || 0;
//                 return sum + (currentLoad - nextLoad);
//             }, 0)
//     };
...
import { useApp } from '../../context/AppContext';
import { Clock, Map, Navigation, CheckCircle, Package } from 'lucide-react';
// We use 'any' here for step temporarily to avoid TypeErrors if your RouteStep interface definition is slightly different
// In a strict setup, update your RouteStep interface to include 'id', 'job', 'delivery', etc.
...
export const RightPanel: React.FC = () => {
    ...
    // --- FIX 1: Simplified Summary Logic ---
    // VROOM usually returns the summary directly. We fallback to 0 if missing.
    // We removed the crashing .filter() code on route.delivery[0]
    const totalDeliveryValue = result.summary?.delivery?.[0] ?? route.delivery?.[0] ?? 0;

    const summary = {
        totalDuration: route.duration || 0,
        totalDistance: route.distance || 0,
        totalDelivery: totalDeliveryValue
    };
                ...
                <div className="p-4">
                    ...
                    <div className="relative border-l-2 border-slate-200 ml-3 space-y-6 pb-6">
                        {route.steps.map((step: any, index: number) => {
                            ...
                            // --- FIX 2: Correct ID access ---
                            // VROOM returns 'id' (int) or 'job' (int) in the JSON. 
                            // It usually does NOT return 'jobId'.
                            // We use step.id || step.job to be safe.
                            const displayId = step.id || step.job || step.jobId || '?';

                            // Calculate drop amount safely
                            const currentLoad = step.load ? step.load[0] : 0;
                            const nextLoad = route.steps[index + 1]?.load ? route.steps[index + 1].load[0] : 0;
                            const dropAmount = currentLoad - nextLoad;
                            ...
                            return (
                                ...
                                ${isStart || isEnd ? 'bg-blue-600' : 'bg-slate-400 group-hover:bg-blue-500'} transition-colors`}
                                    ...
                                    <div className="mt-1">
                                        <h4 className={`text-sm font-medium ${isStart || isEnd ? 'text-blue-800' : 'text-slate-700'}`}>
                                            {isStart ? 'Depart from Depot' : isEnd ? 'Return to Depot' : `Delivery #${displayId}`}
                                        </h4>
                                        <p className="text-xs text-slate-500 mt-0.5 line-clamp-2">
                                            {step.address || `Lat: ${step.location?.[1].toFixed(4)}, Lng: ${step.location?.[0].toFixed(4)}`}
                                        </p>
